...
- 翻译为清单：多个目标语言各自绑定独立快捷键，选中文字一键翻成指定语言
- 输入翻译：没有选中文字时弹出输入面板，左右双栏，边打边译
- 三种翻译服务：百度翻译、AI 大模型（DeepSeek / 智谱 / Kimi 等）、Ollama 本地模型
- 逐句对照：长文按句拆开翻译，悬停高亮对应关系，还能逐句朗读
- 朗读：原文译文都能读，语速可调，系统声音或 Azure；朗读快捷键全部可自定义
- 星标即词典：历史条目加星标，一键生成 AI 词典解读（音标/释义/例句），可朗读、本地保存
- 历史与统计：本地保存可搜索，用量统计（累计/今日/本月、按引擎/按语言），可导出 JSON / CSV
- 界面语言：10 种语言界面，跟随系统或手动指定
- 菜单栏常驻，无 Dock 图标
...
装好后打开「设置 → 翻译引擎」，挑一个服务填好凭据就能用了。

取词方式两个渠道略有不同：官网版首次使用会请求辅助功能权限，用于模拟 `⌘C` 划词取词；App Store 版默认以输入面板取词，如果你自行在系统设置中给它授予了辅助功能权限，划词翻译同样可用——TLKit 不申请也不引导该权限，这完全是你自己的设置。
...
签名凭证优先读环境变量（`APPLE_DEV_TEAM_ID` / `APPLE_DEV_SIGING_NAME` / `APPLE_DEV_PASSWORD`），未注入时回退 1Password CLI，脚本里不落明文。更多实现细节见 [docs/design.md](docs/design.md)。
